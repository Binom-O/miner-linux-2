...
async def get_pow_params(giver_address: str) -> tuple[int, int]:
    try:
        ...
        print(f"An error occurred while fetching POW params for giver {giver_address}: {e}")
        return None, None


async def mine_boc(gpu_id: int, giver_address: str, seed: int, complexity: int) -> tuple[bytes, str] | tuple[
    None, None]:
...
async def mine_all_gpus() -> tuple[any]:
    ...
    for gpu_id in range(config.gpus_count):
        ...
        tasks.append(asyncio.shield(mine_boc(count, giver_address, seed, complexity)))
        count += 1
